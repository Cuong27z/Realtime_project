...
3. Kiến trúc hệ thống (End-to-End)
Fingerprint Sensor / MQTT Command
            ↓
       ESP32 Tasks
   (RMS-like Scheduling)
            ↓
...
Các task logic chính

T_fp – Fingerprint scanning (periodic, high priority)

T_mqtt_srv – MQTT polling server (periodic, high priority)

T_act – Actuator (sporadic, highest priority)

T_ui – LCD & keypad (low priority)

T_net – Network maintenance (best-effort)
